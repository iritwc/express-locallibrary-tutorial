        ...
        if (!errors.isEmpty()) {
            ...
            Book.find({},'title')
                .exec(function (err, books) {
                    if (err) { return next(err); }
                    // Successful, so render.
                    res.render('bookinstance_form', { title: 'Create Book Instance', book_list : books, bookinstance, errors: errors.array(), bookinstance:bookinstance });
                });
            return;
        }
        else {
...
exports.bookinstance_delete_post = function(req, res, next) {
    BookInstance.findById(req.params.id)
        ...
        .exec(function (err, bookinstance) {
            ...
            // Success
            BookInstance.findByIdAndRemove(req.body.bookid, function deleteBookInstance(err) {
                if (err) { return next(err); }
    ...
    function(req, res, next) {
        ...
        if (!errors.isEmpty()) {
            ...
            BookInstance.findByIdAndUpdate(req.params.id, bookinstance, {}, function (err,instance) {
                if (err) { return next(err); }
                // Successful - redirect to instance detail page.
